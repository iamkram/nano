...
function App() {
  ...
  const handleFileSelect = useCallback(async (file: File) => {
    ...
    try {
      const response = await analyzeDocument(file, settings.stylePreset, selectedTemplate || undefined);
      const newPrompt = response.prompt;
      setInputInternal(newPrompt); // Update the main input box with the generated prompt
      ...
      setMessages(prev => [
        ...prev,
        {
          id: Date.now().toString(),
          role: 'assistant',
          content: `Sorry, I encountered an error analyzing the document: ${error instanceof Error ? error.message : 'Unknown error'}. Please try again.`
        }
      ]);
    } finally {
                ...
                {quickPrompts.map((item, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`group flex flex-col gap-4 p-6 rounded-2xl text-left transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-blue-900/10 border ${isSelected
                        ? 'bg-blue-900/20 border-blue-500 shadow-lg shadow-blue-900/20'
                        : 'bg-[#0F1420] hover:bg-[#161b26] border-white/5 hover:border-blue-500/30'
                        }`}
                    >
                      <div className={`p-3 rounded-xl w-fit transition-colors ${isSelected ? 'bg-blue-500 text-white' : 'bg-slate-900/50 text-slate-400 group-hover:bg-blue-500/10 group-hover:text-blue-400'
                        }`}>
                        <item.icon className="w-6 h-6" />
                      </div>
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <h3 className={`font-semibold transition-colors ${isSelected ? 'text-blue-400' : 'text-slate-200 group-hover:text-white'}`}>
                            {item.label}
                          </h3>
                          {isSelected && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-blue-500 text-white uppercase tracking-wide">Active</span>}
                        </div>
                        <p className={`text-sm line-clamp-2 leading-relaxed ${isSelected ? 'text-blue-200/70' : 'text-slate-500 group-hover:text-slate-400'}`}>
                          {item.prompt}
                        </p>
                      </div>
                    </button>
